...
create table member(
	mno int not null unique, 				-- 회원번호 int타입_null[X], 중복[X]
...
#4. 모든 회원들의 정보를 출력한다 --> 테이블내 모든 레코드를 출력한다.				-- 번호와 아이디가 unique라서 1번만 실행 가능 
select * from member;

#5. 모든 회원들의 정보를 제거한다. --> 테이블내 모든 레코드를 제거한다.
delete from member;
	# * mysql workbench에서 delete/update 사용금지
    # 해제 : (메뉴-> edit ->preperence ->SQL Editer 가장아래에서 해제)
    
#6. 회원 아이디 변경한다.		-->테이블내 레코드의 필드값 변경
