...
-- 2. 테이블 : 행/열로 이루어진 표/테이블 [* 테이블 간 관계 기능 있으면 => 관계형 데이터베이스]
...
#1. 레코드(=필드값) 삽입 (= insert into 테이블명 values(값1, 값2, 값3, 값4, 값5)
...
create table member2(
	회원번호1 int,			
    회원번호2 int not null	# 1. not null : 해당필드에 null을 포함하지 않는다.
);

	#1. 레코드(=필드값) 삽입 (= insert into 테이블명 values(값1, 값2, 값3, 값4, 값5)
	#2. insert into 테이블명(특정필드1,특정필드2,특정필드3) values(값1,값2,값3)

insert into member2 values(1,2); 				-- (전체 필드에 값 대입시)회원번호1필드에 1 대입, 회원번호2 필드에 2대입
-- insert into member2 (회원번호1) values(1);		-- 특정 필드에 값 대입시 필드명 명시/작성	오류: 회원번호2 필드가 not null
insert into member2 (회원번호2) values(2);		-- 회원번호1 필드에 null 자동대입, 회원번호2필드에 2대입
...
create table member3(
    ...
    날짜 datetime default now()			# now() 함수는 현재 날짜와 시간을 반환해주느 SQL 함수
	);
insert into member3 values();			#레코드 삽입시 필드값이 없을경우 기본값으로 대입
insert into member3 values(10,20,'일본',false, '2024-01-22 16:44:00');	#레코드삽입시 대입할 필드값이 있을경우 대입
select * from member3;
...
# 5. foreign key(외래키)
	# 1. 다른 테이블의 기본키와 연결/참조된 정보(= 참조)
    

#-----------회원제 게시판 : (회원들이 글을 작성해서 저장) -------------#
# 계층구조: 회원(상위), 게시판(하위)

# 테이블 1개 : 서로다른 목적의 데이터들을 하나의 테이블로 작성시 이상현상(중복/불필요한 정보)발생
# 테이블 2개(정규화) : 동일한 목적의 데이터끼리 테이블을 생성해서 연결/관계(pk-fk) 이용한 관계형 데이터베이스 구축

#5-1 회원 테이블
	#1. 회원번호/2.아이디/3. 비밀번호/ 4.이름
    # 필드별 타입 선정, -> 제약조건 -> pk선정(식별ㄹ키: 중복X ,공백X, 하나의 레코드의 대표!!)
drop table if exists member5;
create table member5(
	mno int 					not null unique,			-- not null이면서 중복없는 필드
	mid varchar(30) 			not null unique,	-- not null이면서 중복없는 필드
	mpw varchar(30)				not null,			-- not null
	mname varchar(20) 			default '익명',		-- insert 할 때 필드의 값이 있을경우 해당값/ 없으면 기본값'익명' 저장
    primary key(mno)								-- mno 필드를 기본키(pk)로 사용	/ 관례적으로 테이블 1개당 pk필드 1개 이상
);

#5-2 게시판 테이블
#1. 게시물번호/ 2.게시물제목/ 3.게시물 내용
drop table if exists board;
create table board(
	bno int 					not null unique,
    btittle varchar(255)		not null,
    bcontent longtext,
    primary key(bno),							-- bno 필드를 기본키(pk)로 사용
    -- 참조 만들기. 게시물이 회원을 참조한다.
    mno_fk int ,									-- 참조테이블에 참조값을 저장할 필드 선정
    foreign key(mno_fk) references member5(mno)
    # foreign key (fk로 사용할 현재 테이블 내 필드) references pk필드테이블명(pk필드)
);
